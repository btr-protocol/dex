...
where:
  `E(t)` is the emission rate at time `t` (in weeks)
  `E₀` is the base emission rate (initial, BTR/week)
  `h` is the halving factor (e.g., 0.5 = 50% reduction per cycle)
  `H` is the halving interval (e.g., 104 weeks = 2 years)
  ⌊·⌋ is the floor function (discrete halving events)
```
...
```mermaid
xychart-beta
    title "Cumulative BTR Emissions over Time"
    x-axis [0, 2, 4, 6, 8, 10, 12, 14]
    y-axis "BTR (Millions)" 0 --> 70
    line [0, 32.5, 49.8, 57.9, 61.9, 63.9, 65, 65]
```
...
1. **Proposal phase** (48-72 hours):
   - Counsel proposes parameter change (e.g., E₀ increase from 62.5k to 65k BTR/week)
...
graph LR
    ...
    subgraph Buyback Mechanism
        Treasury2[Treasury] --> Buy[Buy BTR at market]
...
| **v1.1** | — | — | — | (planned) | Market response adjustments |
...
### 9.1. Public Emission Tracking

**Real-time dashboard** shows:
- **Current emission rate** (BTR/week, per recipient)
- **Cumulative emissions to date**
- **Remaining cap** (e.g., "24.3M of 65M claimed")
- **Weeks until next halving** (countdown)
- **Projected end date** (when all 65M expected to be claimed)
- **Current APY** (by pool, by recipient type)

**Weekly reports** (governance transparency):
- Emissions minted in last week
- Per-pool breakdown
- Fee collection summary
- Treasury balance

### 9.2. Projection Tools

**"What-if" calculator** for governance proposals:
- Input new E₀, H, h
- Outputs: New end date, new annual schedule, APY projections
- Used for community discussion before vote
...
1. **Emergency vote** (Counsel can initiate without waiting)
...
### 10.2. Cap Adjustment (Supermajority Only)
...
### 11.2. Distributor Integration

**Contract**: `DistributorV1.sol`
...
### 11.3. SDK Emission Calculations

**Location**: `sdk/src/emissions/` (future implementation)

Provides:
- `projectedEmissions(E₀, H, h, t)` — Emission at time t
- `halvingSchedule()` — Full schedule as JSON
- `apyProjection(tvl)` — APY curve over time
