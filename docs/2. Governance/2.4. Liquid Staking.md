...
$$
V_{btr} = sBTR_{balance} \times btr_{price} \times govMultiplier / 10000
$$

where:
- $govMultiplier$ = 10000 (default, meaning 1x weight)
- $btr_{price}$ = price of BTR at snapshot block
...
- Bridgeable cross-chain via `BridgeV1` (see [BridgeV1.sol](/contracts/src/BridgeV1.sol))
...
$$
Reward_{asset} = Total_sLP_{emissions} \times weight_{pool} \times weight_{asset}
$$

where:
- $weight_{pool}$ ∝ $TVL_{pool} / Total_{TVL}$
- $weight_{asset}$ ∝ $(coverage \times utilization)_{asset}$
...
$$
V_{total} = V_{btr} + V_{lp} + V_{boost}
$$

where:
- $V_{btr}$ = voting value from sBTR = $sBTR_{balance} \times BTR_{price} \times govMultiplier / 10000$
- $V_{lp}$ = voting value from LP = $\sum(sLP_{balance} \times LP_{price} \times lpBaseMultiplier / 10000)$ across all pools
- $V_{boost}$ = boosted voting value = $\min(V_{lp}, boostCap \times V_{btr} / 10000) \times lpBoostedMultiplier / 10000$
