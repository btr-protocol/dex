...
**Key principle**: Treasury is **owned by the DAO Counsel (multisig)**, which executes decisions approved by BTR token holders through governance voting.

---

## 2. Treasury Structure & Ownership

### 2.1. Ownership Model

- **Primary owner**: DAO Counsel multisig (initially 5-7 signers)
- **Transition path**: Counsel → Full DAO governance (voting power-based) by Year 2
- **Ownership transfer**: Subject to 7-day timelock + 7-day grace period

### 2.2. Spending Authorities
...
- Counsel can execute without additional vote
...
- **Flash loan fees**: 0.009% (90 basis points) per borrowed amount
- **Withdrawal haircuts**: Coverage-dependent fees on low-collateral withdrawals
...
**Governance process**:
1. Counsel proposes annual budget (Q4 of previous year)
...
**Adjustment process**:
1. Counsel proposes emission parameter change
...
**Transparency dashboard** (available to community):
- Current treasury balance (token + LP position value)
- Spending history (last 30 days, last year, all-time)
- Team vesting progress (% unlocked, total claimed)
- Emissions progress (% claimed of 65M cap)

### 8.2. Governance Thresholds

| Action | Approval Threshold | Timelock | Notes |
|--------|-------------------|----------|-------|
| **Monthly spending** | Counsel only | None | Capped at 2% of supply |
...
1. **Multisig emergency execution**: Counsel can trigger emergency backup
...
- **Bridge**: May relay multi-chain earnings to treasury (future V2)
...
## 12. Treasury Dashboard & Transparency

**Public endpoints** (future):
- Treasury balance (real-time)
- Spending history (CSV export)
- Team vesting tracker (per-beneficiary)
- Emissions progress (curve vs actual)

**Community governance tools**:
- Treasury Snapshot space for spending proposals
- Multisig signature tracking (etherscan/arbiscan)
- Fee collection analytics (pool-by-pool breakdown)
