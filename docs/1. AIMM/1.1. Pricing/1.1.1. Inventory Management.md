...
```mermaid
stateDiagram-v2
    [*] --> Undercollateralized: c < 1.0
    [*] --> Equilibrium: c = 1.0
    [*] --> Overcollateralized: c > 1.0

    Undercollateralized --> Critical: c <= 0.5
    Undercollateralized --> Equilibrium: deposit improves c

    Critical --> Undercollateralized: decay or deposit
    Critical --> [*]: max withdrawal penalty

    Equilibrium --> Undercollateralized: withdrawals
    Equilibrium --> Overcollateralized: deposits

    Overcollateralized --> Equilibrium: withdrawals incentivized

    note right of Critical
        Max premium (skew = +100)
        Withdrawal haircuts active
        Decay may activate
    end note
    ...
    note right of Overcollateralized
        Max discount (skew = -100)
        Selling incentivized
    end note
```
...
### 2.3. Coverage vs Skew Visualization

```mermaid
graph LR
    subgraph Coverage["Coverage Ratio"]
        C200["200%<br/>c=2.0"]
        C150["150%<br/>c=1.5"]
        C100["100%<br/>c=1.0"]
        C75["75%<br/>c=0.75"]
    ...
    subgraph Skew["Inventory Skew"]
        S100["+100<br/>Max Premium"]
        S50["+50<br/>Premium"]
        S0["0<br/>Equilibrium"]
        SN50["-50<br/>Discount"]
        SN100["-100<br/>Max Discount"]
    end
    ...
    style S100 fill:#90EE90
    style S0 fill:#87CEEB
    style SN100 fill:#FFB6C1
```
...
```mermaid
graph TD
    Start([User requests withdrawal of x LP tokens]) --> CalcC{Calculate coverage c = R/L}
    CalcC -->|c >= 1| NoHaircut[No haircut - full withdrawal]
    CalcC -->|c < 1| CalcH[Calculate h = 1 - c^p]

    NoHaircut --> UpdateR[R = R - x]
    CalcH --> CalcY[Calculate y = x * 1 - h]

    CalcY --> UpdateR2[R = R - y]
    UpdateR --> UpdateL[L = L - x]
    UpdateR2 --> UpdateL

    UpdateL --> Receive[User receives y tokens]
    Receive --> End([Complete])

    style CalcH fill:#FFB6C1
    style CalcY fill:#FFB6C1
    style NoHaircut fill:#90EE90
```
...
- $I$ = $\sum_j p_j * |R_j - L_j|$ sum of imbalances across assets
