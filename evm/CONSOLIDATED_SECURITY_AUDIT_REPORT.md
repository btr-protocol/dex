...
**Severity**: CRITICAL  
**Component**: `InternalOracle.sol`, `LibOracle.sol`, `Base.sol`
...
// In InternalOracle.sol, add constants:
...
// In Flash, before callback:
assembly {
    tstore(FLASH_LOAN_ACTIVE_SLOT, 1)
}

// In InternalOracle.pushFeedInternal(), add check:
...
**Severity**: CRITICAL  
**Component**: `Base.sol`, `Exchange.sol`, `Liquidity.sol`, `Flash.sol`
...
1. Attacker initiates swap in Exchange
2. During swap, oracle callback triggers external call
3. Attacker re-enters via different module (e.g., Liquidity)
...
- `Exchange.sol`: swap(), batchSwap(), getSwapQuote()
- `Liquidity.sol`: deposit(), donate(), withdraw(), swapLiability()
- `Flash.sol`: flashLoan()
- `Staking.sol`: stakeGov(), unstakeGov(), stakeLP(), unstakeLP()
- `Distributor.sol`: claimCampaign()
- `Rescue.sol`: rescueToken()
...
**Severity**: CRITICAL  
**Component**: `LibTimelock.sol`, `Admin.sol`, `PoolProxy.sol`
...
```solidity
// Admin.sol lines 100-131 (executeAddAsset)
...
function executeAddAsset(address token) external override onlyOwner {
    ...
    delete $.pendingData[id];
    emit IAdmin.AssetAdded(tokenNorm, decimals, 0);
}
```

**Apply to**: All `execute*()` functions in `Admin.sol`
...
**Identified By**: Smith (Red Tactics), Deckard (Blue), Trinity (Validation)  
**Severity**: HIGH  
**Component**: `Liquidity.sol`, `Exchange.sol`, `Base.sol`
...
**Apply to**: All `_pull()` calls in `Base.sol`
...
**Identified By**: Smith (Red Tactics), Deckard (Blue), Trinity (Validation)  
**Severity**: HIGH  
**Component**: `PoolProxy.sol`, All modules
...
struct PoolStorage {
    // Core fields (existing from IPool)
    mapping(bytes4 => address) modules;
    mapping(bytes32 => bool) trustedModules;
    address owner;
    
    // Slot 0-9: Core state (Exchange, Base, etc.)
    uint128 baseReserve;
    ...
    // Slot 10-19: LP state (Liquidity)
    uint256 lpTotalSupply;
    mapping(address => uint256) lpBalances;
    
    // Slot 20-29: Oracle state (InternalOracle)
    Observation[] observations;
    mapping(address => IOracle.OracleAccumulator) accumulators;
    
    // Slot 30-39: Staking state (Staking)
    ...
    // Slot 40-49: Bridge state (Bridge)
    address bridge;
    mapping(address => IBridge.TokenConfig) tokenConfigs;
    mapping(bytes32 => IBridge.FailedMessage) failedMessages;
    
    // Slot 50-99: Treasury state (Treasury)
    address govToken;
    uint256 maxSupply;
    uint32 tgeTimestamp;
...
**Required Fix**: Add overflow checks to LP calculations in `Liquidity.sol`
...
| Module Upgrade Trust Bypass | Smith | Kusanagi, Trinity | Trust check exists in PoolProxy |
...
|----|---------|----------|------|---------------|
| CRITICAL-1 | TWAP Oracle Manipulation | New | InternalOracle.sol | ~200-300 |
| CRITICAL-2 | Global Reentrancy Guard | Existing | Base.sol | ~27-44 |
| CRITICAL-3 | Timelock Bypass | New | Admin.sol | ~100-131 |
| CRITICAL-4 | Death Spiral Coverage | New | LibPricing.sol | ~76-130 |
| CRITICAL-5 | Unchecked Token Returns | Partial | Base.sol | ~191-208 |
| CRITICAL-6 | Bridge Authorization Race | New | Bridge.sol | ~95-129 |
| CRITICAL-7 | Integer Overflow | New | LibPricing.sol | ~823-1025 |
| CRITICAL-8 | Anchor Path Manipulation | New | LibAnchorTree.sol | ~84-129 |
| CRITICAL-9 | Governance Capture | New | Admin.sol | ~266-286 |
| CRITICAL-10 | Storage Collision | Existing | PoolProxy.sol | ~40-48 |
| HIGH-1 | Fee-on-Transfer | New | Base.sol | ~191-208 |
| HIGH-2 | Storage Collision Docs | New | PoolProxy.sol | ~40-48 |
| HIGH-3 | LP Dilution | New | Liquidity.sol | ~24-90 |
| HIGH-4 | Sandwich Skew | New | Exchange.sol | ~22-54 |
| HIGH-5 | Withdrawal Queue | New | Liquidity.sol | ~104-173 |
| HIGH-6 | Fee Extraction | New | Admin.sol | ~160-179 |
| HIGH-7 | Cross-Asset Manipulation | New | LibAnchorTree.sol | ~84-129 |
| HIGH-8 | LP Supply Overflow | New | Liquidity.sol | ~48-78 |
| HIGH-9 | Oracle Stale Check | Existing | InternalOracle.sol | ~65-92 |
| HIGH-10 | Emergency Withdraw | Existing | Rescue.sol | ~20-40 |
| HIGH-11 | WETH9 Reentrancy | New | Exchange.sol | ~30, 117 |
| HIGH-12 | Cross-Module Hooks | New | Base.sol, IPoolHooks.sol | ~163-188 |
| HIGH-13 | Admin Timelock Integration | New | Admin.sol | ~274-285 |
| HIGH-14 | Oracle Flash Manipulation | New | InternalOracle.sol | ~200-300 |
| MEDIUM-1 | JIT Flow Guard | New | Base.sol | ~84-103 |
...
| MEDIUM-8 | Stale Oracle Exploitation | New | InternalOracle.sol | ~200-300 |
| MEDIUM-9 | Callback Reentrancy | New | Multiple modules | - |
| MEDIUM-10 | Flash Loan Inventory | New | Flash.sol | ~23-84 |
| MEDIUM-11 | Governance Propagation | New | PoolProxy.sol | ~462-500 |
| MEDIUM-12 | TWAP Timestamp | New | InternalOracle.sol | ~200-300 |
| MEDIUM-13 | Token Decimals | New | LibPricing.sol | ~1-1146 |
| MEDIUM-14 | LayerZero Trust | New | Bridge.sol | ~95-129 |
| MEDIUM-15 | Treasury Race | New | Treasury.sol | ~231-242 |
| MEDIUM-16 | Pool Proxy Delegatecall | New | PoolProxy.sol | ~208-231 |
...
- Module trust system (PoolProxy)
- Failed message recovery (Bridge)
