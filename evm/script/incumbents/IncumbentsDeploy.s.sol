...
contract IncumbentsDeploy is Script {
    ...
    function _deployCurve3(Tokens memory t, uint256 seed) internal returns (address pool) {
        ...
        amts[1] = seed;
        amts[2] = seed;
        for (uint256 i; i < 3; i++) _approve(coins[i], pool, seed);
    ...
    function _deployAllCL(LiteCLFactory clf, Tokens memory t, uint256 seed) internal returns (uint256 n) {
    ...
    function _deployFluid(FluidDexFactory ff, Tokens memory t, uint256 seed) internal returns (uint256 n) {
