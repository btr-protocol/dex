...
contract SafetyOps is Script {
  ...
  function pauseAll() external {
    _fleet(IAdmin.BatchOp.Pause);
  }

  function unpauseAll() external {
    _fleet(IAdmin.BatchOp.Unpause);
  }

  function freezeAll() external {
    _fleet(IAdmin.BatchOp.Freeze);
  }

  function unfreezeAll() external {
    _fleet(IAdmin.BatchOp.Unfreeze);
  }

  function pauseAsset(address token) external {
    _asset(token, IAdmin.BatchOp.Pause);
  }

  function unpauseAsset(address token) external {
    _asset(token, IAdmin.BatchOp.Unpause);
  }

  function freezeAsset(address token) external {
    _asset(token, IAdmin.BatchOp.Freeze);
  }

  function unfreezeAsset(address token) external {
    _asset(token, IAdmin.BatchOp.Unfreeze);
  }
