...
/// @title ArcPoolDeploy — Arc Testnet (5042002) DEX stack on top of the already-deployed oracle.
/// @notice Core singletons (reusing the oracle stack's AccessControl) + faucet + a reference
///         ExternalOracle + two launch pools, both USDC-based:
///           stable core = USDC, USDT, USDE, USDS, USD1, USDG, PYUSD, RLUSD (trimmed Arc roster)
///           fx core     = USDC, EURC, QCAD, AUDF, BRLA, JPYC, KRW1 (deployFxPool ceremony)
///         No volatile core on Arc. USDC is the real 6-decimal Circle token from the SoT (not a
///         TestnetERC20 mock). wnative=0 (Arc has no WETH9; Pool.initialize accepts address(0)).
///         Set WNATIVE unset or 0; do not point at a placeholder.
/// @dev TWO JSON INPUTS, both consumed, nothing hardcoded:
///      1. `deployments/5042002.deploy.json` (SoT, written by SepoliaOracleDeploy): `ac`,
...
///      2. `deployments/arc-risk-params.json` (written by
...
/// @dev Env: DEPLOYER_PK, GUARDIAN (required). WNATIVE optional on Arc: unset or address(0) for
///      wnative=0 (no native wrap). If set nonzero, must be a live ERC-20 matching SoT WETH (Arc
///      SoT carries no WETH; leave unset). Optional: DEPLOY_IN (oracle SoT
...
/// @dev RUNBOOK: `forge script script/ArcPoolDeploy.s.sol:ArcPoolDeploy --sig
///      "deployPools()" --rpc-url arc` (simulate) then re-run with --broadcast --slow.
...
///      from 5042002.pools.json). keepers/oracle.sepolia.toml must carry a [reference] block with
///      refOracle's address + the refFeeds names; keepers/scripts/fill-oracle-config.py resolves
///      each name to a feed id by keccak(token, USDC) off 5042002.deploy.json (same as primary).
///      Start the keeper on both in the same session the pools go live.
contract ArcPoolDeploy is Deploy {
  ...
  // derivation + per-leg sigma live in deployments/arc-risk-params.json `basis.fx`.
  ...
  struct Cfg {
    string sot; // raw deployments/5042002.deploy.json
    string risk; // raw deployments/arc-risk-params.json
  ...
  /// @dev One row of deployments/arc-risk-params.json, resolved for a single symbol.
  struct AssetParams {
    address token;
    uint16 presetId;
    uint16 minFeePbps;
  ...
  function deployPools() external returns (address stablePool, address volatilePool) {
    ...
    require(
      ...
      "already deployed: 5042002.pools.json carries a live stablePool (REDEPLOY=true to override)"
    );
    ...
    if (volSyms.length > 0) {
      ...
      if (volSyms.length > 0) {
        ...
        console2.log("SKIP volatile pool: no listable volatile symbols");
      }
    } else {
      console2.log("SKIP volatile pool: Arc ships no volatile core");
    }
  ...
  function _loadCfg() internal view returns (Cfg memory cfg) {
    ...
    // `seedUsdc` holds the USD notional in 1e18 (seedUsdPerLeg × 1e18), NOT a token raw amount.
    // `_seedRaw` converts via IERC20.decimals() so Arc's 6d USDC and 18d mocks both size correctly.
  ...
  /// @dev Live ERC-20 decimals. Arc USDC is 6; TestnetERC20 mocks are 18. Never hardcode.
  ...
  /// @dev Convert USD notional (1e18) at ceremony mark into token raw units: amt = usd1e18 · 10^d / mark.
  ...
  /// @dev WNATIVE is written to Pool storage at initialize. Arc has no WETH9: unset or address(0)
  ///      is valid (Pool.initialize accepts wnative=0; native wrap stays disabled). If set
  ///      nonzero, it must be a live ERC-20 matching the SoT WETH leg (Sepolia-style mock).
  function _resolveWnative(string memory sot) internal view returns (address w) {
    w = vm.envOr("WNATIVE", address(0));
    if (w == address(0)) return address(0);
  ...
  function _listAsset(
    ...
    admin.addAsset(
      ...
      p.presetId,
      p.minFeePbps,
      _decimals(p.token), // Arc USDC=6; mocks=18. Must match IERC20.decimals() (audit A4-01).
      p.minDisp,
      p.maxDisp,
      uint16(cfg.gamma),
      uint16(cfg.vega)
  ...
  function _seedPool(
    ...
    for (uint256 i; i < tokens.length; ++i) {
      ...
      _creditDeployer(cfg, recipient, tokens[i], amt, true);
      IERC20(tokens[i]).approve(poolAddr, type(uint256).max);
      pool.deposit(tokens[i], amt);
    }
  }

  /// @dev TestnetERC20 exposes `minter()`; real USDC (and other non-mocks) do not.
  function _isTestnetMintable(address token) internal view returns (bool) {
    (bool ok, bytes memory ret) = token.staticcall(abi.encodeWithSignature("minter()"));
    if (!ok || ret.length < 32) return false;
    return abi.decode(ret, (address)) != address(0);
  }

  /// @dev Mint TestnetERC20 mocks; otherwise require the deployer already holds the tokens (Arc USDC).
  ///      Returns false when `required=false` and balance is insufficient (faucet skip).
  function _creditDeployer(
    Cfg memory cfg,
    address recipient,
    address token,
    uint256 amt,
    ...
    if (_isTestnetMintable(token)) {
      TestnetERC20(token).mint(recipient, amt);
      return true;
    }
    if (IERC20(token).balanceOf(recipient) < amt) {
      if (!required) {
        if (token == cfg.usdc) {
          console2.log(
            "SKIP faucet USDC: deployer balance insufficient (Circle faucet funds gas USDC separately)"
          );
        } else {
          console2.log("SKIP faucet fund: deployer balance insufficient for non-mintable token", token);
        }
        return false;
      }
      revert("deployer balance insufficient for non-mintable token");
    }
    return true;
  }
  ...
  ///      Bound: 10^p · mark ≤ target_1e18 · 10^decimals (USDC 6d vs mock 18d both fit).
  ...
  function _fundFaucet(
    ...
    for (uint256 i; i < k; ++i) {
      ...
      address recipient = vm.addr(vm.envUint("DEPLOYER_PK"));
      if (!_creditDeployer(cfg, recipient, t, amt, false)) continue;
  ...
  //   1. mintFxTokens  — the mock ERC20s (18 dp). Listing + seed read IERC20.decimals(); Arc USDC
  //      is 6d at the real Circle address, so hardcoding 18 here would misprice the base by 1e12.
  ...
  function _riskPath() internal view returns (string memory) {
    return vm.envOr("RISK_PARAMS", string("deployments/arc-risk-params.json"));
  }
  ...
  /// @dev Core, faucet and the REFERENCE oracle are all REUSED from 5042002.pools.json — a third
  ...
  function _poolsOutPath() internal view returns (string memory) {
    ...
    // 5042002.deploy.json would destroy the token/feed bindings the keeper reads.
    return vm.envOr(
  ...
  function _persist(
    ...
    // addresses in 5042002.deploy.json, identically to the primary. See runbook step "REF KEEPER".
