...
import {Rescue} from "../src/modules/Rescue.sol";
...
contract DeployBSCFork is Script {
    ...
    function deployUserContract(
        ...
        if (useCreate3) {
            bytes memory initCode = abi.encodePacked(type(PoolProxy).creationCode);

            address deployed = createX.deployCreate3(salt, initCode);
            console2.log("%s: %s", logName, deployed);
            return deployed;
        } else {
            address deployed = address(new PoolProxy());
    ...
    function deployFactory(bytes32 salt, address expectedAddr, bool useCreate3) internal returns (address) {
        if (useCreate3) {
            ...
            bytes memory initCode = abi.encodePacked(
                type(PoolProxyFactory).creationCode,
            ...
            PoolProxyFactory deployed = new PoolProxyFactory(referencePool, deployer);
    ...
    function deployRouter(bytes32 salt, address expectedAddr, bool useCreate3) internal returns (address) {
        if (useCreate3) {
            bytes memory initCode = abi.encodePacked(type(Router).creationCode);

            address deployed = createX.deployCreate3(salt, initCode);

            // Initialize router
            Router(deployed).initialize(deployer, factory);

            console2.log("Router: %s", deployed);
            return deployed;
        } else {
            Router deployed = new Router();
    ...
    function deployPoolZero() internal returns (address) {
        ...
        // Deploy via factory
        address poolAddr = PoolProxyFactory(payable(factory)).createPool(
    ...
    function deployModules() internal {
        ...
        flashModule = address(new Flash());
        rescueModule = address(new Rescue());
    ...
    function _refreshFeeds(address pool, bool isPoolZero) internal {
        for (uint i = 0; i < tokens.length; i++) {
            if (isPoolZero ? tokens[i].inPoolZero : tokens[i].inPoolStable) {
                ...
                InternalOracle(pool).updateFeed(tokens[i].addr, price, accDec, 10000, 10000);
            }
        }
    }

    function _getDefaultFeeParams() internal pure returns (IPool.FeeParams memory) {
        uint8[29] memory pad;
        return IPool.FeeParams({
            protoShare: 25,
            flashFeeBps: 5,
            _pad: pad
        });
    ...
    function concentratedProfile() internal pure returns (IPool.LiquidityProfile memory p) {
        p.weights[0] = 10;
        p.weights[1] = 180;
        p.weights[2] = 10;
        p.knots[0] = -50;
    ...
    function balancedProfile() internal pure returns (IPool.LiquidityProfile memory p) {
        p.weights[0] = 50;
        p.weights[1] = 100;
        p.weights[2] = 50;
        p.knots[0] = -50;
