...
contract StableSwapPool {
    ...
    function add_liquidity(uint256[] calldata amounts, uint256 minMint) external returns (uint256 mintAmount) {
        ...
        for (uint256 i; i < N; i++) balances[i] = newBalances[i];
        totalSupply += mintAmount;
        lpBalances[msg.sender] += mintAmount;
        emit AddLiquidity(msg.sender, amounts, mintAmount);
    }

    function remove_liquidity(uint256 amount, uint256[] calldata minAmounts) external returns (uint256[] memory out) {
    ...
    function _getD(uint256[] memory xp) internal view returns (uint256 d) {
        uint256 s;
        for (uint256 i; i < N; i++) s += xp[i];
        if (s == 0) return 0;
        d = s;
        // Ann = A · N^N (Curve StableSwap)
        uint256 ann = A;
        for (uint256 i; i < N; i++) ann *= N;
    ...
    function _getY(uint256 i, uint256 j, uint256 x, uint256[] memory xp, uint256 d) internal view returns (uint256 y) {
        uint256 ann = A;
        for (uint256 k; k < N; k++) ann *= N;
        uint256 c = d;
        uint256 s;
        for (uint256 k; k < N; k++) {
            if (k == j) continue;
