...
interface IUniswapV2FactoryFee {
  function feeTo() external view returns (address);
}

/// @dev Vendored from Uniswap V2 `UniswapV2Pair.sol` (v1.0.1).
///      Accounting / 0.3% fee / K invariant identical; SafeMath → 0.8 arithmetic.
contract UniswapV2Pair is UniswapV2ERC20 {
  ...
  function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
    address feeTo = IUniswapV2FactoryFee(factory).feeTo();
  ...
  function skim(address to) external lock {
    address _token0 = token0;
    address _token1 = token1;
    _safeTransfer(_token0, to, _balanceOf(_token0) - reserve0);
    _safeTransfer(_token1, to, _balanceOf(_token1) - reserve1);
  }
  ...
  function _balanceOf(address token) private view returns (uint256) {
    ...
    require(ok && data.length >= 32, "UniswapV2: BALANCE_FAILED");
