...
library FluidDexMath {
    ...
    function imaginaryReserves(
        ...
        uint256 upperRange = (centerPrice * FOUR_DECIMALS) / (FOUR_DECIMALS - upperPercent);
        ...
        if (gp < PRICE_PRECISION) {
            (token0Imag, token1Imag) = calculateReservesOutsideRange(gp, upperRange, token0Real, token1Real);
        } else {
