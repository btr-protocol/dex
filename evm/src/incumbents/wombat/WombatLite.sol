...
contract WombatLite {
    ...
    function deposit(address token, uint256 amount, uint256 minLp) external returns (uint256 lpMinted) {
    ...
    function withdraw(address token, uint256 lpAmount, uint256 minOut) external returns (uint256 amountOut) {
        if (!isToken[token]) revert UnknownToken();
    ...
    function quote(address fromToken, address toToken, uint256 fromAmount) public view returns (uint256 toAmount) {
