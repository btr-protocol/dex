...
contract RangeCLPool is IUniSpot {
    ...
    function _liqForAmount0(uint160 sqrtA, uint160 sqrtB, uint256 amount0_) internal pure returns (uint128) {
        if (sqrtA > sqrtB) (sqrtA, sqrtB) = (sqrtB, sqrtA);
        uint256 intermediate = (uint256(sqrtA) * uint256(sqrtB)) / SqrtPrice.Q96;
        return uint128((amount0_ * intermediate) / (sqrtB - sqrtA));
    }

    function _liqForAmount1(uint160 sqrtA, uint160 sqrtB, uint256 amount1_) internal pure returns (uint128) {
        if (sqrtA > sqrtB) (sqrtA, sqrtB) = (sqrtB, sqrtA);
