...
contract PoolFactory is IPoolFactory {
  ...
  constructor(address referencePool_, address protocolDeployer_, address ac_) {
    if (referencePool_ == address(0) || protocolDeployer_ == address(0) || ac_ == address(0)) {
      revert Err.ZeroValue();
    }
    if (referencePool_.code.length == 0 || ac_.code.length == 0) revert Err.NotCode();
    IPool refPool = IPool(referencePool_);
    _validateImplementation(referencePool_, ac_, refPool.admin(), refPool.flash());
  ...
  function createPool(address baseToken, address[] calldata tokens, bytes calldata initdata)
    external
    returns (address pool)
  {
    ...
    bytes32 salt = keccak256(
      abi.encodePacked(msg.sender, baseToken, keccak256(abi.encode(tokens)), block.chainid)
    );
  ...
  function _registerPool(address pool, address creator, address baseToken, address[] memory tokens)
    internal
  {
    isPool[pool] = true;
    allPools.push(pool);
  ...
  function _addTokens(address pool, address[] memory tokens, bool official) internal {
    ...
    if (poolToTokens[pool].length + n > MAX_POOL_TOKENS) {
      revert Err.ExcessiveAmount(n, MAX_POOL_TOKENS);
    }
    for (uint256 i; i < n;) {
      ...
      if (!tokenInPool[token][pool]) {
        ...
        if (!official && tokenToPools[token].length >= MAX_TOKEN_POOLS) {
  ...
  function requestReferenceUpgrade(address newImplementation) external onlyAdmin {
    ...
    _validateImplementation(newImplementation, AC, IPool(live).admin(), IPool(live).flash());
  ...
  function _validateImplementation(
    ...
    if (
      implementation.code.length == 0 || ac_.code.length == 0 || admin_.code.length == 0
        || flash_.code.length == 0
    ) revert Err.NotCode();
    if (IAdmin(admin_).AC() != ac_) revert Err.BadConfig();
    IPool candidate = IPool(implementation);
    address auxAddr = candidate.poolAux();
    if (
      candidate.AC() != ac_ || candidate.admin() != admin_ || candidate.flash() != flash_
        || auxAddr.code.length == 0
