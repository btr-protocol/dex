...
import "./ChainMeta.sol";

/// @title BNBChainMeta
abstract contract BNBChainMeta is ChainMeta {
    function __id() public pure override returns (string memory) {
        return "bnb_chain";
    }

    function __expectedChainIds() public pure override returns (uint256[] memory ids) {
        ids = new uint256[](2);
        ids[0] = 56;     // BNB Chain mainnet
        ids[1] = 31337;  // local fork
    }

    function __tokens() public pure virtual override returns (TokenMeta memory t) {
