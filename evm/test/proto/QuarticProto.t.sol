...
library NUQuartic {
    ...
    function pushConvert(Curve storage c, uint256 xspan, uint256[] memory interior, int256[] memory wQ)
        internal
    {
        ...
        returns (uint256[] memory t)
    {
        ...
        for (uint256 i = n; i < n + 5; ++i) t[i] = xspan;
    }
        ...
        returns (int256)
    {
        ...
        return S + (int256(t[j + 5] - t[j + 4]) * (60 * k[0] + 30 * k[1] + 20 * k[2] + 15 * k[3] + 12 * k[4])) / 60;
    }
    ...
    {
        int256[5] memory d;
        for (uint256 k = 0; k < 5; ++k) d[k] = wQ[s - 4 + k];
    ...
    {
        int256[4] memory d;
        for (uint256 k = 0; k < 4; ++k) d[k] = _q(t, wQ, s - 4 + k);
