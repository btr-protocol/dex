...
///      `IPool.FeeParams feeParams` packs into slot 10; flowCooldownSeconds is slot 11.
///      Hooks append-only: assetHooks=15, invested=16 (SDK `POOL_STORAGE`).
contract PoolStorageLayoutTest is Test {
  ...
  uint256 constant ASSET_HOOKS_SLOT = 15;
  uint256 constant INVESTED_SLOT = 16;
  ...
  /// @notice Pin hooks mapping bases (assetHooks=15, invested=16) to SDK `POOL_STORAGE`.
  function test_layout_hooks_mapping_bases() public {
    ...
    assertEq(h.target, SENTINEL_HOOK, "assetHooks base must be slot 15");
    ...
    assertEq(
      IPool(address(pool)).getInvested(SENTINEL_TOKEN), 42e18, "invested base must be slot 16"
    );
  }
}
