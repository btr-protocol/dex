...
///      curves=6, lpBalances=7, protocolFees=8). Off-chain readers (SDK storage.ts)
...
///      Tail (append-only): factory=13, assetHooks=14, invested=15 (SDK `POOL_STORAGE`).
contract PoolStorageLayoutTest is Test {
  ...
  uint256 constant LP_BALANCES_SLOT = 7;
  uint256 constant PROTOCOL_FEES_SLOT = 8;
  uint256 constant ASSET_HOOKS_SLOT = 14;
  uint256 constant INVESTED_SLOT = 15;
  ...
  /// @notice Pin hooks mapping bases (assetHooks=14, invested=15) to SDK `POOL_STORAGE`.
  function test_layout_hooks_mapping_bases() public {
    ...
    assertEq(h.target, SENTINEL_HOOK, "assetHooks base must be slot 14");
    ...
    assertEq(
      IPool(address(pool)).getInvested(SENTINEL_TOKEN), 42e18, "invested base must be slot 15"
    );
  }
  ...
  function test_layout_renumbered_mapping_bases() public {
    ...
    assertEq(IPool(address(pool)).getAsset(SENTINEL_TOKEN).reserves, 7e18, "assets base must be slot 3");
    ...
    assertEq(IPool(address(pool)).getProtocolFees(SENTINEL_TOKEN), 123e6, "protocolFees base must be slot 8");
    // oracleConfigs=4 / riskConfigs=5 / curves=6 / lpBalances=7 are asserted as compile-time
    // constants matching the SDK POOL_STORAGE map; the two probes above bracket the renumbered range.
    ...
    assertEq(LP_BALANCES_SLOT, ASSETS_SLOT + 4, "lpBalances=7");
  }
}
