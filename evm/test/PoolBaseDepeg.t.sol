...
contract MockBaseOracle {
    ...
    function getFeed(bytes32) external view returns (IOracle.FeedData memory feed) {
        ...
        feed.fastVolEMA = 1_000_000;
        feed.slowVolEMA = 1_000_000;
    ...
    function getFastEMA(bytes32) external view returns (uint64) { return priceB64; }
}
...
contract PoolBaseDepegTest is Test {
    ...
    function _skipped_test_R44_2b_design_doc() public {
        ...
        uint64 px = M.encodeB64(1e18, 18);
        vm.prank(OWNER);
        admin.addAsset(address(pool), address(cTok), oc, rc, pf, 1000, 18, px, 10_000, 10_000, 1000, 100000, 10000, 10000, 10000);
        // Seed C.
