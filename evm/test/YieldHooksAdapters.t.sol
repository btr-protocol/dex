...
contract YieldHooksAdaptersTest is BaseTestSetup {
  ...
  function test_sweepIncentives_skips_cToken() public {
    MockVenus vToken = new MockVenus(address(quote));
    CompoundV2YieldHook cHook = new CompoundV2YieldHook(
      address(ac), address(pool), address(quote), address(vToken), address(0), bytes4(0)
    );
  ...
  function test_aaveV4_experimental_deploy_and_recall() public {
    ...
    AaveV4YieldHook hook = new AaveV4YieldHook(
      address(ac), address(pool), address(quote), address(spoke), reserveId, address(0)
    );
    _setHook(address(quote), address(hook), hook.recommendedFlags());
    ...
    returns (AaveV3YieldHook hook, MockERC20 reward)
  {
    ...
    hook =
      new AaveV3YieldHook(address(ac), address(pool), address(quote), address(aave), address(rc));
    _setHook(address(quote), address(hook), hook.recommendedFlags());
  }
  ...
  function _merklData(address[] memory tokens, uint256[] memory amounts)
    internal
    pure
  ...
  function _aaveHookWith(address controller) internal returns (address) {
    ...
    aave.setAToken(address(quote), address(aToken));
    return address(
      new AaveV3YieldHook(address(ac), address(pool), address(quote), address(aave), controller)
    );
  }
  ...
  function _compoundHookWith(address comptroller, bytes4 selector) internal returns (address) {
    ...
    return address(
      new CompoundV2YieldHook(
        address(ac), address(pool), address(quote), address(v), comptroller, selector
      )
    );
  }
  ...
  function test_incentives_aaveFamily_multiReward_all_swept() public {
    ...
    MockAaveRewardsMulti ctrl =
      new MockAaveRewardsMulti(_addrs2(address(a), address(b)), _u2(12e18, 7e18));
    a.mint(address(ctrl), 12e18);
    b.mint(address(ctrl), 7e18);
    _expectClaimSweep(
      _aaveHookWith(address(ctrl)), "", _addrs2(address(a), address(b)), _u2(12e18, 7e18)
    );
  }
  ...
  function test_incentives_compoundFamily_selectors() public {
    bytes4[3] memory sels = [
      MockComptroller.claimVenus.selector,
      MockComptroller.claimComp.selector,
      MockComptroller.claimReward.selector
    ];
    for (uint256 k; k < 3; ++k) {
      ...
      _expectClaimSweep(
        _compoundHookWith(address(comp), sels[k]), "", _addrs1(address(rwd)), _u1(25e18)
      );
    }
  }
  ...
  function test_incentives_compoundFamily_gating() public {
    ...
    _assertGating(
      _compoundHookWith(address(comp), MockComptroller.claimVenus.selector), "", address(rwd), 9e18
    );
  }
