...
contract Phase42DTest is Test {
    ...
    function test_R3_A4_1_applyOffset_fullMulDiv_noOverflow_atExtremeInputs() public pure {
        ...
        int256 multiplier = int256(LibOracle.ORACLE_PBPS) + int256(int32(type(int32).max));
        // expected = spot * multiplier / ORACLE_PBPS, exact via mulDiv.
        uint256 expected = FixedPointMathLib.fullMulDiv(spot, uint256(multiplier), LibOracle.ORACLE_PBPS);
        // Sanity: result is large but well-defined.
        assertGt(expected, 0);
        // Re-encoding through mulDiv is consistent with the LibOracle._applyOffset path.
    ...
    function test_R3_A4_1_encodeOffset_roundTrip_unchanged() public pure {
        ...
        int32 off = LibOracle.encodeOffset1e18(spot, twap);
        // Manual decode using same formula as _applyOffset (fullMulDiv equiv at small inputs).
        int256 multiplier = int256(LibOracle.ORACLE_PBPS) + int256(off);
        uint256 decoded = FixedPointMathLib.fullMulDiv(spot, uint256(multiplier), LibOracle.ORACLE_PBPS);
