...
abstract contract CoverageProofsBase is Test {
  ...
  function setUp() public virtual {
    ...
    vm.stopPrank();
    base.mint(address(this), 10 * SEED);
    tok.mint(address(this), 10 * SEED);
...
contract CoverageProofsTest is CoverageProofsBase {
  ...
  function _cache(uint128 r, uint128 l, uint16 kappa)
    internal
    pure
    returns (Pricing.EndpointCache memory c)
  {
    c.reserves = r;
    c.liabilities = l;
    c.kappaCovBps = kappa;
  ...
  function test_fuzz_toll_wall_blocks_full_drain(uint128 r0, uint128 l, uint256 g, uint16 kappa)
    public
    pure
  {
  ...
  function test_fuzz_toll_charge_only_overcovered(uint128 l, uint256 g, uint16 kappa) public pure {
    ...
    assertEq(
      Pricing._covToll(_cache(r0, l, kappa), g), 0, "over-covered drain toward peg must be free"
    );
  }

  /// Lemma C monotonicity: T non-decreasing in gross size.
  function test_fuzz_toll_monotone_in_size(
    uint128 r0,
    uint128 l,
    uint256 g1,
    uint256 g2,
  ...
  function test_fuzz_coverage_floor_single_drain(uint128 r0, uint128 l, uint256 g, uint16 kappa)
    public
    pure
  {
    (r0, l, g) = _boundDrain(r0, l, g);
  ...
  function test_fuzz_coverage_floor_sequence(
    uint128 r0,
    uint128 l,
    uint256 g1,
    uint256 g2,
