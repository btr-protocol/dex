...
contract PoolDecayHarness {
  ...
  function setAsset(address token, uint128 reserves, uint128 liabilities, uint32 lastUpdate)
    external
  {
...
contract PoolDecayTest is Test {
  ...
  function test_calculateDecay_deficitCap() public view {
    ...
    uint128 r = h.callCalculateDecay(uint128(1000e18), 0, 65535, type(uint32).max, type(uint32).max);
