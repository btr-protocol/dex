...
contract InternalOracleTest is Test {
    ...
    function setUp() public {
        ...
        oracle.setFeed(bytes32(uint256(uint160(address(stable)))), M.encodeB64(1e18, 18), C.STABLE_SIGMA, 0, 3600);
        oracle.setFeed(REF_ID, M.encodeB64(1e18, 18), C.STABLE_SIGMA, 0, 3600);

        vm.startPrank(OWNER);
        admin.addAsset(
            address(pool), address(base), _oracleExt(address(base)), _riskStable(), _profile(),
            1000, 18, 1000, 100000, 10000, 10000
        );
        admin.addAsset(
            address(pool), address(stable), _internalOracle(), _riskStable(), _profile(),
            1000, 18, 1000, 100000, 10000, 10000
        );
        vm.stopPrank();
    ...
    function test_walk_off_peg_quotes_constant_peg() public {
        oracle.setFeed(bytes32(uint256(uint160(address(stable)))), M.encodeB64(995e15, 18), C.STABLE_SIGMA, 0, 3600);
        oracle.setFeed(REF_ID, M.encodeB64(1e18, 18), C.STABLE_SIGMA, 0, 3600);
    ...
    function test_band_halt_when_gate_depegs() public {
        oracle.setFeed(bytes32(uint256(uint160(address(stable)))), M.encodeB64(101e16, 18), C.STABLE_SIGMA, 0, 3600);
