...
contract SplineDumpTest is Test {
    ...
    returns (S.Point[] memory pts)
  {
    ...
    for (uint256 i; i < weights.length; ++i) {
      cumW += weights[i];
      pts[i + 1] = S.Point({
        x: (cumW * BPS) / WEIGHT_SUM, y: (int256(knots[i + 1]) * int256(uint256(disp))) / 100
      });
    }
  }
  ...
  function _dump(string memory label, S.Point[] memory pts) internal view {
    for (uint256 i; i < pts.length; ++i) {
      console2.log(
        string.concat("KNOT,", label, ",", vm.toString(pts[i].x), ",", vm.toString(pts[i].y))
      );
    }
    // Fine grid over the WHOLE domain, step=25 (401 points).
    for (uint256 x; x <= BPS; x += 25) {
      int256 v = S.eval(x, pts);
      console2.log(string.concat("EVAL,", label, ",", vm.toString(x), ",", vm.toString(v)));
    }
  ...
  function test_dump_flat_segment_disp1000() public view {
    ...
    assertEq(
      S.area(pts, 2500, 5000),
      flatY * int256(5000 - 2500),
      "flat segment area must equal exact rectangle"
    );
  }
