...
- **BAMM Protocol**: Balanced Automated Market Maker - a multi-asset DEX with hub-and-spoke routing, dynamic fees, and coverage ratio-based active liquidity management
- **Web Interface** ([https://btr.supply](https://btr.supply)): Front-end for interacting with the BAMM protocol
...
**Impermanent Divergence**: Unlike traditional AMMs, BAMM uses single-sided liquidity (deposit USDC, withdraw USDC). While this eliminates classic impermanent loss, coverage ratio fluctuations can cause similar effects if reserves deplete via swaps.
