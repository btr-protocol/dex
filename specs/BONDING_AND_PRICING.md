...
BAMM uses a **piecewise linear bonding curve** (liquidity profile) instead of traditional invariant functions (like Uniswap's `x*y=k` or Curve's complex invariant). This design is inspired by concentrated liquidity (Uniswap V3) but optimized for gas efficiency with configurable segments rather than tick-based liquidity.

**Key Components:**
1. **Liquidity Profiles**: Piecewise linear bonding curves (this document)
...
## Fee Multiplier Considerations

### Threshold Smoothness

BAMM applies **tri-factor fee multipliers** (volatility × inventory × divergence) to the base swap fee. These multipliers use tiered/step-wise thresholds that create **discontinuities**.

**Example discontinuity** (coverage multiplier):
- At 4.99% pool weight: fee multiplier = 5.0x
- At 5.00% pool weight: fee multiplier = 3.0x
- **Jump: -2.0x discontinuity**

### Impact on Trading

**Positive:**
- Clear, predictable fee brackets
- Easy to understand and communicate
- Simple implementation

**Negative:**
- Traders can game threshold crossings
- Sudden fee jumps may surprise users
- MEV opportunities at boundaries

### Mitigation Options

1. **Option A (Recommended)**: Linear interpolation between thresholds
2. **Option B**: Quadratic/exponential smoothing
3. **Option C (Current)**: Keep step-wise with explicit documentation

### Testing Requirements

Threshold boundary fuzz tests (fee changes at 4.99% → 5.01%) validate fee transitions. Monotonicity tests ensure fees do not increase as liquidity increases. Gaming profitability tests verify split trades are not cheaper than single trades. MEV simulation tests check for sandwich attack vulnerabilities around threshold boundaries.

**Related:**
- [FEES.md](./FEES.md) - Tri-factor fee model overview
