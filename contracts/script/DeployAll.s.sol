...
/// @title DeployAll
/// @notice Complete deployment script for BAMMCore + DarkPool infrastructure
contract DeployAll is Script {
    function run() external {
        uint256 deployerPrivateKey = vm.envUint("PRIVATE_KEY");
        address deployer = vm.addr(deployerPrivateKey);

        console2.log("=== Deploying Complete BAMMCore + DarkPool Infrastructure ===");
        ...
        // ========== STEP 4: Deploy BAMMCore Infrastructure ==========
        console2.log("");
        console2.log("Step 4: Deploying BAMMCore infrastructure...");

        BAMMCore bammImpl = new BAMMCore();
        console2.log("  BAMMCore implementation:", address(bammImpl));

        BAMMFactory bammFactory = new BAMMFactory(address(bammImpl), deployer);
        console2.log("  BAMMFactory:", address(bammFactory));
        console2.log("  BAMMCore Beacon:", address(bammFactory.beacon()));

        // ========== STEP 5: Configure BAMMCore Factory with DarkPool ==========
        console2.log("");
        console2.log("Step 5: Configuring BAMMCore Factory for DarkPool...");
        ...
        console2.log("3. Deploy BAMMCore pools with deployPool(..., enableDarkPool=true)");
