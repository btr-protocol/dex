...
abstract contract BaseTestSetup is Test {
    ...
    function makeFeedData(
        ...
        uint32 fastVolEMA,
        uint32 slowVolEMA
    ) internal view returns (IOracleV1.FeedData memory) {
    ...
    function getSigma(IOracleV1.FeedData memory feed) internal pure returns (uint32) {
        return LibOracle.getSigma(feed);
    }

    /// @notice Get delta (deviation) from feed data
    function getDelta(IOracleV1.FeedData memory feed) internal pure returns (uint32) {
