...
4. **Deviation band (SOURCE-time-driven per-push sanity cap)**: `devBps <= maxDeviation*(1 + dtSource/ttl)` capped `MAX_DEV_THRESHOLD` (65_000), where `dtSource = (sourceTs - prevSourceTs)/1000` seconds of ATTESTED source time. Chain-agnostic: identical %/wall-second allowance on a 400ms and a 12s chain; a fast chain gets no extra cumulative walk from more blocks. On the first signed push (`prevSourceTs == 0`), `dtSource` is defined as zero, so the configured `maxDeviation` applies with no bootstrap widening. This is a per-push SANITY cap only; the CUMULATIVE bound on a compromised push quorum is the independent `refPrimary` band (below).
...
`mapping(address => bool) signers` + packed `uint8 signerThreshold`/`uint8 signerCount`. Construction atomically installs 3–6 unique non-zero signers and requires `2 <= signerThreshold <= signerCount`; there is no single-signer deployment state. `grantSigner` (owner) / `revokeSigner` (guardian OR owner) keep `signerCount` in sync (idempotent), with a hard six-signer cap. `setSignerThreshold(t)` (owner) requires `2 <= t <= signerCount`, so governance can never recreate 1-of-n authority. Revoking below the threshold is deliberately allowed: it HALTS pushing (fail-safe compromise response); feeds go stale and the pool fail-closes.
