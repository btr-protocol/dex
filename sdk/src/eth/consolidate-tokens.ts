...
/**
 * Script to consolidate TOKEN_ADDRESSES and TOKENS into a single clean structure
 */
...
const addressesMatch = content.match(/export const TOKEN_ADDRESSES[\s\S]*?^};/m);
...
export const QUOTE_TOKENS = ['USDC', 'USDT', 'ETH', 'BTC', 'DAI', 'USDE'];
...
export function tokenMatchesSearch(symbol: string, searchTerm: string): boolean {
  ...
  // Direct match
  if (symbol.toUpperCase().includes(searchUpper)) return true;

  // Check if any token with wrapperOf=symbol matches the search
  for (const [tokenSymbol, token] of Object.entries(TOKENS)) {
    if (token.wrapperOf === symbol && tokenSymbol.includes(searchUpper)) {
      return true;
    }
  }
