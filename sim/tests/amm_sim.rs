//! Cross-validation of the production-aligned Rust AIMM settlement core plus explicitly named
//! research variants. Integration test (links the lib; independent of unrelated in-tree WIP tests).
...
fn uncrossed_market_high_vol() {
    ...
    assert!(
        ask > 3000.0 && bid < 3000.0,
        "ask {ask} / bid {bid} must bracket twap 3000"
    );
}
...
fn buy_size_monotonic() {
    ...
    assert!(
        big > small,
        "bigger buy must pay more: big {big} <= small {small}"
    );
}
...
fn balanced_quotes_near_twap() {
    ...
    assert!(
        ask >= 2000.0 && bid <= 2000.0,
        "ask {ask} >= 2000 >= bid {bid}"
    );
    assert!(
        (ask - 2000.0).abs() < 60.0 && (2000.0 - bid).abs() < 60.0,
        "spread sane: ask {ask} bid {bid}"
    );
}

#[test]
fn research_only_signals_are_not_reported_as_production_aimm() {
    let production = Aimm::new(AimmParams::default(), 2_000.0, 1_000_000.0)
        .with_mode(OracleMode::External { interval: 1 });
    assert_eq!(production.name(), "AIMM");

    let research_params = AimmParams {
        lambda: 1.0,
        ..AimmParams::default()
    };
    let research = Aimm::new(research_params, 2_000.0, 1_000_000.0)
        .with_mode(OracleMode::External { interval: 1 });
    assert_eq!(research.name(), "AIMM-Research");
}

/// The router consumes `quote`, then executes `swap`; both must include the same production-ordered
/// coverage toll. This also pins Solidity ordering: gross output -> charge-only wall -> path spread.
#[test]
fn coverage_wall_quote_matches_swap_and_solidity_order() {
    let price = 2.0;
    let p = AimmParams {
        gamma: 0.0,
        vega: 0.0,
        ...
        min_fee: 2_000.0,
        max_fee: 2_000.0,
        kappa_cov: 0.01, // 100 bps, matching the deployed stable preset's unit conversion
        kappa_convex: true,
        ..AimmParams::default()
    };
    let mut a = Aimm::new(p, price, 1_000.0).with_mode(OracleMode::External { interval: 1 });
    a.tok_res = 0.7 * a.tok_liab;

    let amount_in = 100.0;
    let gross = amount_in / price;
    let q = |c: f64| c.ln() - c + 1.0;
    let c0 = a.tok_res / a.tok_liab;
    let c1 = (a.tok_res - gross) / a.tok_liab;
    let toll = (q(c0) - q(c1)) * 0.01 * a.tok_liab;
    let expected = (gross - toll) * (1.0 - 2_000.0 / (2.0 * 1_000_000.0));

    let quoted = a.quote(0, 1, amount_in);
    let executed = a.swap(0, 1, amount_in);
    assert!(
        (quoted - expected).abs() < 1e-12,
        "quote {quoted} != ordered settlement {expected}"
    );
    assert!(
        (executed - quoted).abs() < 1e-12,
        "router-visible quote {quoted} != swap {executed}"
    );
}

/// A token sell restores token coverage but outputs the untolled base leg. Production is
/// charge-only and has no rebate ledger, so execution is exactly the ordinary spread-adjusted fill.
#[test]
fn coverage_wall_restoring_trade_has_no_rebate() {
    let price = 2.0;
    let p = AimmParams {
        gamma: 0.0,
        vega: 0.0,
    ...
    let quoted = a.quote(1, 0, 10.0);
    let expected = 10.0 * price * (1.0 - 2_000.0 / (2.0 * 1_000_000.0));
    assert!(
        (quoted - expected).abs() < 1e-12,
        "restoring quote {quoted} != no-rebate fill {expected}"
    );
    assert!((a.swap(1, 0, 10.0) - quoted).abs() < 1e-12);
}
...
fn compare_aimm_vs_univ3_gbm() {
    ...
    for r in [&ra, &rv] {
        assert!(
            r.net_apr.is_finite() && r.lvr_apr.is_finite(),
            "{} metrics finite",
            r.name
        );
...
fn oracle_frequency_sweep() {
    ...
    let mut a_hy = mk().with_mode(OracleMode::Hybrid {
        alpha: 0.3,
        band: 0.05,
    });
    ...
    assert!(
        best.0 > internal_net + 0.5,
        "external push should beat internal by >50%/yr; best {:.2} internal {:.2}",
        best.0,
        internal_net
    );
}

/// CHARGE-ONLY WALL SEMANTICS. Starting below peg, the production wall prevents additional output-leg
/// drain but does not manufacture a restoring-flow rebate. This test guards against reviving the old
/// simulator-only claim that the Solidity wall actively re-pegs inventory.
#[test]
fn coverage_wall_prevents_further_drain_without_claiming_repeg() {
    let p0 = 3000.0;
    let days = 10.0;
    ...
    let mut walled = mk(0.5, 0.7);
    let rw = engine::run(&mut walled, &flat, &cfg, days);

    println!(
        "\n  charge-only wall (start c=0.70, flat px):\n  skew-only : c_final={:.4}  mean|c-1|={:.4}\n  walled    : c_final={:.4}  mean|c-1|={:.4}",
        rs.cov_final, rs.cov_mean_dev, rw.cov_final, rw.cov_mean_dev
    );

    assert!(
        rs.cov_final < 0.7,
        "unwalled flow should be able to worsen coverage, got {:.4}",
        rs.cov_final
    );
    assert!(
        rw.cov_final >= 0.7 - 1e-9,
        "wall should prevent further drain, got {:.4}",
        rw.cov_final
    );
    assert!(
        rw.cov_final > rs.cov_final,
        "wall should preserve more coverage than skew-only"
    );
}

/// RE-PEG NON-EXTRACTION (the property the old uniform mark-shift FAILED). The path-integrated
/// charge-only potential toll must be non-extractable: an off-peg pool at a frozen mark, hit by a sell-then-buy
...
/// conservative form's core guarantee (restoring flow is untolled but never receives a rebate).
#[test]
fn repeg_toll_not_round_trip_extractable() {
    ...
    let mk = || {
        let p = AimmParams {
            kappa_cov: 1.0,
            kappa_convex: true, // convex wall (the strong regime)
            ..AimmParams::default()
        };
        let mut a = Aimm::new(p, p0, 10_000_000.0).with_mode(OracleMode::External { interval: 1 });
    ...
    // sell-first round trip: sell token (improves c, no rebate) then buy it back (worsens c).
    let mut a1 = mk();
    let base_got = a1.swap(1, 0, notional_tok);
    let tok_back = a1.swap(0, 1, base_got);
    assert!(
        tok_back <= notional_tok + 1e-6,
        "sell-first round trip extracted: {tok_back} > {notional_tok}"
    );

    // buy-first round trip: buy token (worsens c, charged by the wall) then sell it back (improves,
    // but receives no rebate).
    let mut a2 = mk();
    ...
    assert!(
        base_back <= base_seed + 1e-6,
        "buy-first round trip extracted: {base_back} > {base_seed}"
    );
}
...
fn convex_wall_resists_drain() {
    ...
    assert!(
        rc.cov_final > 0.5,
        "convex wall should hold coverage > 0.5, got {:.4}",
        rc.cov_final
    );
    assert!(
        (rc.cov_final - 1.0).abs() < 0.2,
        "convex should stay near peg, got {:.4}",
        rc.cov_final
    );
}
...
fn honest_lvr_cadence_sweep() {
    ...
    let mk_aimm = || -> Box<dyn Amm> {
        Box::new(
            Aimm::new(AimmParams::default(), p0, tvl / 2.0)
                .with_mode(OracleMode::External { interval: 1 }),
        )
    };
    ...
    println!(
        "\n  HONEST LVR vs keeper cadence (60% vol, 25s fine steps, {}-seed avg):",
        seeds.len()
    );
    ...
    assert!(
        fresh_lvr < v3_lvr,
        "fresh-cadence AIMM LVR {:.4} should be < UniV3 {:.4}",
        fresh_lvr,
        v3_lvr
    );
    assert!(
        stale_lvr > fresh_lvr,
        "staleness must raise AIMM LVR ({:.4} stale vs {:.4} fresh)",
        stale_lvr,
        fresh_lvr
    );
}
...
fn surcharge_decoupled_defends_pickoff() {
    ...
    let run_avg = |lambda: f64| -> (f64, f64) {
        ...
        for &sd in &seeds {
            ...
            let p = AimmParams {
                lambda, // 0 = no adverse-selection surcharge; >0 = Δ-keyed surcharge
                ..AimmParams::default()
            };
    ...
    assert!(
        surcharge_lvr <= no_surcharge_lvr + 1e-9,
        "Δ-surcharge must not raise LVR ({:.4} vs {:.4})",
        surcharge_lvr,
        no_surcharge_lvr
    );
}
...
fn stablecoin_competitive() {
    let path = concat!(
        ...
        "/../research/data/stables/univ3_usdc_usdt_1bp.csv"
    );
    ...
    let ap = AimmParams {
        kappa_cov: 0.05,
        kappa_convex: true,
        min_fee: 30.0, // 0.3 bps
        ..AimmParams::default()
    };
    let field = || -> Vec<Box<dyn Amm>> {
        vec![
            Box::new(
                Aimm::new(ap.clone(), p0, tvl / 2.0)
                    .with_mode(OracleMode::External { interval: 1 }),
            ),
    ...
    println!(
        "\n  === STABLECOIN COMPETITION (real Uni-V3 USDC/USDT, {:.0}d, mean|depeg| {:.2}bp) ===",
        days, mean_dev
    );
    println!("  AMM            net/CMix   LVR     vol_share  trader_cost");
    for r in &reps {
        println!(
            "  {:<12} {:>8.3}% {:>7.3}% {:>8.1}%  {:>8.3}bps",
            r.name,
            r.net_apr_cm * 100.0,
            r.lvr_apr * 100.0,
            r.won_share * 100.0,
    ...
    assert!(
        aci.net_apr_cm >= curve.net_apr_cm,
        "AIMM-CI should beat Curve on net-vs-CM: {:.3}% vs {:.3}%",
        aci.net_apr_cm * 100.0,
        curve.net_apr_cm * 100.0
    );
    assert!(
        aci.won_share >= curve.won_share,
        "AIMM-CI should win >= Curve's stable flow: {:.3} vs {:.3}",
        aci.won_share,
        curve.won_share
    );
    for r in &reps {
        assert!(
            r.trader_cost_bps.is_finite() && r.net_apr_cm.is_finite(),
            "{} finite",
            r.name
        );
    }
...
fn fine_competitive_10s() {
    ...
    let p = concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../research/data/fine/ETH-USDT.bars"
    );
    ...
    for r in &reps {
        println!(
            "  {:<12} {:>8.2}% {:>8.2}% {:>7.2}% {:>8.1}%  {:>8.2}bps",
            r.name,
            r.net_apr_hodl * 100.0,
            r.net_apr_cm * 100.0,
            r.lvr_apr * 100.0,
            r.won_share * 100.0,
            r.trader_cost_bps
    ...
    assert!(
        aci.net_apr_cm >= cc.net_apr_cm,
        "AIMM-CI should beat CryptoSwap on net-vs-CM: {:.3}% vs {:.3}%",
        aci.net_apr_cm * 100.0,
        cc.net_apr_cm * 100.0
    );
    assert!(
        aci.trader_cost_bps <= cc.trader_cost_bps + 1e-9,
        "AIMM-CI should be tighter than CryptoSwap: {:.2} vs {:.2}bps",
        aci.trader_cost_bps,
        cc.trader_cost_bps
    );
}
...
fn staleness_premium_sweep() {
    ...
    let days = (bf.records().last().unwrap().open_time_ms() - bf.records()[0].open_time_ms())
        as f64
        / 86_400_000.0;
    let tvl = 10_000_000.0;
    let p0 = prices[0];
    ...
    println!(
        "\n  === STALENESS-PREMIUM SWEEP (real 10s ETH, {:.1}d, 30s keeper) ===",
        days
    );
    ...
    for &z in &[0.0f64, 100.0, 1_000.0, 10_000.0, 100_000.0] {
        ...
        let a = &reps[0];
        println!(
            "  z={:<6.0}  {:>8.2}% {:>8.2}% {:>7.2}% {:>8.1}%  {:>8.2}bps",
            z,
            a.net_apr_hodl * 100.0,
            a.net_apr_cm * 100.0,
            a.lvr_apr * 100.0,
            a.won_share * 100.0,
            a.trader_cost_bps
    ...
    assert!(
        best_lvr < base_lvr * 0.25 + 1e-9,
        "staleness premium should collapse LVR: best {:.2}% vs base {:.2}%",
        best_lvr * 100.0,
        base_lvr * 100.0
    );
}
...
fn deviation_triggered_push() {
    ...
    println!(
        "\n  === DEVIATION-TRIGGERED PUSH (real 10s ETH, {:.1}d, 1h heartbeat) ===",
        days
    );
    ...
    for &th_bp in &[1.0f64, 5.0, 10.0] {
        ...
        let ap = AimmParams {
            min_fee: 2.0 * th_bp * 100.0, // half-spread ≥ θ  (θ bp → 2θ bp floor, in PBPS)
            min_disp: 100.0,
            stale_z: 2.0, // small residual for the keeper reaction / heartbeat-tail window
            ..AimmParams::default()
        };
        let mut amms: Vec<Box<dyn Amm>> = vec![
            Box::new(
                Aimm::new(ap, p0, tvl / 2.0).with_mode(OracleMode::Deviation {
                    threshold: th,
                    heartbeat: 360,
                }),
            ),
        ...
        println!(
            "  θ={:>4.0}bp   {:>8.2}% {:>8.2}% {:>7.2}% {:>8.1}%  {:>8.2}bps   n/a",
            th_bp,
            a.net_apr_hodl * 100.0,
            a.net_apr_cm * 100.0,
            a.lvr_apr * 100.0,
        ...
        assert!(
            a.lvr_apr < 0.30,
            "deviation-push LVR should stay bounded at θ={th_bp}bp: {:.2}%",
            a.lvr_apr * 100.0
        );
...
fn deviation_push_budget() {
    ...
    let tvl = 10_000_000.0;
    println!(
        "\n  === DEVIATION-PUSH KEEPER BUDGET (real 10s, θ={:.0}bp volatile, 1h heartbeat) ===",
        th_bp
    );
    println!(
        "  asset      days   pushes  push/day   2y_pushes   2y_cost$   net/HODL   LVR    won%"
    );
    let mut any = false;
    for s in syms {
        let p = format!(
            "{}/../research/data/fine/{}.bars",
            env!("CARGO_MANIFEST_DIR"),
            s
        );
        let Ok(bf) = BarFile::open(std::path::Path::new(&p)) else {
        ...
        let p0 = prices[0];
        let ap = AimmParams {
            min_fee: 2.0 * th_bp * 100.0, // half-spread ≥ θ
            stale_z: 2.0,
            ..AimmParams::default()
        };
        let mut amms: Vec<Box<dyn Amm>> = vec![
        ...
        let a = &reps[0];
        println!(
            "  {:<9} {:>5.1}  {:>7}  {:>8.1}  {:>10.0}  {:>9.0}  {:>8.2}%  {:>5.2}%  {:>5.1}%",
            s,
            days,
            pushes,
            per_day,
            two_yr,
            two_yr * cost_per_push,
            a.net_apr_hodl * 100.0,
            a.lvr_apr * 100.0,
            a.won_share * 100.0
        );
    ...
    if any {
        println!(
            "  (2y_cost at ${:.3}/push — Base est; scale linearly for your gas. Wider θ ⇒ fewer pushes ⇒ cheaper, but wider quotes.)",
            cost_per_push
        );
    } else {
    ...
    if let Ok(bf) = BarFile::open(std::path::Path::new(&p)) {
        ...
        let p0 = prices[0];
        println!(
            "\n  θ cost-curve (ETH, {:.0}d): θ(bp)  push/day   2y_pushes   2y_cost$   net/HODL   LVR    won%",
            days
        );
        for &tb in &[5.0f64, 10.0, 20.0, 50.0] {
            let ap = AimmParams {
                min_fee: 2.0 * tb * 100.0,
                stale_z: 2.0,
                ..AimmParams::default()
            };
            let mut amms: Vec<Box<dyn Amm>> = vec![
            ...
            let a = &reps[0];
            println!(
                "            {:>5.0}  {:>8.1}  {:>10.0}  {:>9.0}  {:>8.2}%  {:>5.2}%  {:>5.1}%",
                tb,
                per_day,
                per_day * 730.0,
                per_day * 730.0 * cost_per_push,
                a.net_apr_hodl * 100.0,
                a.lvr_apr * 100.0,
                a.won_share * 100.0
            );
...
fn fresh_oracle_vs_cfmm() {
    ...
    let p0 = prices[0];
    let mut amms: Vec<Box<dyn Amm>> = vec![
        // aligned spline+coverage+oracle AIMM (Pricing.sol mirror: charge-only wall, NO research
        // Δ-surcharge — lambda=0), FRESH external keeper (interval:1), at the demand-optimal spread
        // = the default min_fee 1000 (10bp full ⇒ 5bp/side) + min_disp 1000. The old 1bp floor was
        // only optimal while the research lambda term padded revenue; under the production spread
        // (S_vol + staleness + confidence only) the sweep plateau is min_fee 700-1500.
        Box::new(
            Aimm::new(AimmParams::default(), p0, tvl / 2.0)
                .with_mode(OracleMode::External { interval: 1 }),
        ),
    ...
    let reps = router::run_competitive(&mut amms, &prices, &cfg, days);
    println!(
        "\n  === FRESH oracle-quote AIMM vs CFMMs (real 10s ETH, {:.0}d, 10s keeper) ===",
        days
    );
    println!("  AMM            net/HODL  net/CMix   LVR     vol_share  trader_cost");
    for r in &reps {
    ...
    // THE REALIGNED WIN: the production-aligned (charge-only, no Δ-surcharge) spline+coverage+oracle
    // AIMM, at fresh cadence + the demand-optimal default spread, beats the CFMM (CryptoSwap) on LP
    // economics AND has ~0 LVR (a fresh oracle-quote prevents the LVR a CFMM can't avoid). No
    // invariant swap needed — the architecture is right; the fix was keeper freshness + spread calibration.
    let aimm = reps.iter().find(|r| r.name == "AIMM").unwrap();
    let cc = reps.iter().find(|r| r.name == "CurveCrypto").unwrap();
    assert!(
        aimm.lvr_apr <= cc.lvr_apr + 1e-9,
        "fresh AIMM LVR {:.3} should be <= CFMM {:.3}",
        aimm.lvr_apr,
        cc.lvr_apr
    );
    assert!(
        aimm.net_apr_cm >= cc.net_apr_cm,
        "fresh AIMM net/CM {:.3} should beat CFMM {:.3}",
        aimm.net_apr_cm,
        cc.net_apr_cm
    );
    for r in &reps {
...
fn multi_asset_capital_efficiency() {
    ...
    for s in syms {
        ...
        let prices: Vec<f64> = recs.iter().map(|r| r.close).filter(|&c| c > 0.0).collect();
        let days =
            (recs[recs.len() - 1].open_time_ms() - recs[0].open_time_ms()) as f64 / 86_400_000.0;
    ...
    for (sym, prices, days) in &paths {
        let p0 = prices[0];
        let mut a =
            Aimm::new(AimmParams::default(), p0, d).with_mode(OracleMode::External { interval: 1 });
        ...
        println!(
            "  {:<10} fee_apr {:>7.2}%  net {:>7.2}%  lvr {:>6.2}%",
            sym,
            fee_apr * 100.0,
            r.net_apr * 100.0,
            r.lvr_apr * 100.0
    ...
    println!(
        "\n  === CAPITAL EFFICIENCY ({} pairs, D=${:.0}M/pair) ===",
        paths.len(),
        d / 1e6
    );
    println!(
        "  isolated (fragmented base): capital ${:.0}M -> LP fee APR {:.2}%",
        iso_cap / 1e6,
        iso_apr * 100.0
    );
    println!(
        "  hub-spoke (unified base):   capital ${:.0}M -> LP fee APR {:.2}%",
        hub_cap / 1e6,
        hub_apr * 100.0
    );
    println!(
        "  => hub is {:.2}x more capital-efficient (same fees, {:.0}% the capital)",
        hub_apr / iso_apr.max(1e-9),
        hub_cap / iso_cap * 100.0
    );
    // the hub earns the same fees on strictly less capital ⇒ strictly higher LP APR.
    assert!(
        hub_apr > iso_apr,
        "hub LP APR {:.4} must exceed isolated {:.4}",
        hub_apr,
        iso_apr
    );
    assert!(
        hub_cap < iso_cap,
        "hub capital must be less than fragmented"
    );
}
...
fn aimm_spread_floor_competitive() {
    ...
    assert!(
        reps[floors.len() - 1].won_share >= reps[0].won_share,
...
fn real_2yr_competitive() {
    let path = concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../research/data/ohlc/ETH-USDT.csv"
    );
...
fn competitive_field_benchmark() {
    ...
    for &competitive in &[false, true] {
        ...
        println!(
            ...
            if competitive {
                "COMPETITIVE (best-execution)"
            } else {
                "NAIVE (same flow to all)"
            },
            tvl / 1e6
        );
        println!("  AMM            net/HODL  net/CMix   LVR     vol_share  trader_cost");
        ...
        for r in &reps {
            assert!(
                r.net_apr_hodl.is_finite() && r.trader_cost_bps.is_finite(),
                "{} finite",
                r.name
            );
        }
        if competitive {
            ...
            let winner = reps
                .iter()
                .max_by(|a, b| a.won_share.partial_cmp(&b.won_share).unwrap())
                .unwrap();
            ...
            assert!(
                v2.won_share < 0.5,
                "fat-fee UniV2 must not dominate competitive flow: {:.3}",
                v2.won_share
            );
            assert!(
                winner.trader_cost_bps < 5.0,
                "competitive flow winner ({}) should offer tight execution, got {:.2}bps",
                winner.name,
                winner.trader_cost_bps
            );
        }
    }
}
...
fn trade_offset_discovery_comparison() {
    ...
    let params = || {
        AimmParams {
            min_fee: 2_000.0, // 20 bp full spread => 10 bp one-sided protection = theta
            ..AimmParams::default()
        }
    };
    ...
    for &gain in &[0.25, 0.5, 1.0, 2.0] {
        for &max_offset in &[0.0005, 0.001, 0.002] {
            for &size in &[100.0, 1_000.0, 10_000.0, 100_000.0] {
                ...
                let mut plain =
                    Aimm::new(p.clone(), p0, tvl / 2.0).with_mode(OracleMode::Deviation {
                        threshold: 0.001,
                        heartbeat: 10,
                    });
                ...
                assert!(
                    plain_pnl <= 1e-6,
                    "baseline round trip unexpectedly profitable"
                );
            }
        }
    }
    ...
    assert!(
        profitable > 0,
        "falsification sweep must expose unsafe trade-offset settings"
    );
}
