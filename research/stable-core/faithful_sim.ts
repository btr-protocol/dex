...
const COVMIN=+(Bun.env.COVMIN||5_000);
const SHARED = { gamma:10_000, vega:10_000, lambda:10_000, minDisp:1_000, maxDisp:100_000, covMin:COVMIN, covMax:20_000, depthAmp:10_000, protoShare:20, weights:[50,50,50,50], knots:[-50,-25,0,25,50] };
...
const REBAL_COST_BP=0.25, BAND_LO=0.80, BAND_HI=1.20, TURN=8.0;
const MINFEE=+(Bun.env.MINFEE||100);
const TVLS=(Bun.env.TVLS||'50000,100000,250000,500000,1000000').split(',').map(Number);
...
async function run(tvl:number){
  ...
  let t0=0,t1=0;
  const stream=Bun.file('out/tape_3mo.csv').stream(); const dec=new TextDecoder(); let buf='';
  let first=true;
...
for(const tvl of TVLS){ const r=await run(tvl); out.push(r);
