...
import type { AgentConfig, SessionContext, SessionStats, Storage } from '@shared/types';
import { logger } from '@btr/sdk/utils';
import { agentsDBPath } from '@shared/storage';
import { ensureStorageDirectories } from './search/storage-init.js';
...
class StorageAdapter implements Storage {
  ...
  constructor(dbPath: string = agentsDBPath) {
    this.dbPath = dbPath;
  }
  ...
  async registerAgent(config: AgentConfig): Promise<void> {
    ...
    if (existing) {
      ...
      log.info(`Agent ${config.agentId} updated`);
    } else {
      this.db!.prepare(
      ...
      log.info(`Agent ${config.agentId} registered`);
    }
  }
  ...
  async createSession(sessionId: string, agentId: string, userId?: string): Promise<void> {
    ...
    log.info(`Session ${sessionId} created for agent ${agentId}`);
  }
  ...
  async compactSession(
    ...
    log.info(`Session ${sessionId} compacted: ${stats.beforeTokens} -> ${stats.afterTokens} tokens`);
  }
