...
const log = logger.withContext('collector-server');
...
export class MarketDataServer {
  ...
  constructor(collector: MarketCollector, port: number = 3000, rpcUrl?: string) {
    ...
    log.info('[server] Initializing with RPC URL:', rpcUrl);
    if (rpcUrl) {
      this.poolService = new PoolDataService(rpcUrl);
      log.info('[server] PoolDataService created');
      this.poolService.loadPoolAddresses().catch(log.error);
    } else {
      log.info('[server] No RPC URL provided - pool service will not be available');
    }
  }
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      if (pid) {
        ...
        log.info(`Cleaned up port ${this.port} (PID: ${pid})`);
      }
    } catch (e) {
    ...
    this.server = Bun.serve({
      ...
      async fetch(req: Request, bunServer: any) {
        ...
        if (url.pathname === '/api/pools') {
          log.info('[/api/pools] Handler called, poolService:', !!poolService);
          if (!poolService) {
            log.info('[/api/pools] ERROR: poolService is not available!');
            return new Response(
              JSON.stringify({ error: 'Pool service not configured' }),
              { status: 503, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
            );
          }
          log.info('[/api/pools] Fetching pool data...');
    ...
    log.info('\nEndpoints:');
    log.info(`  REST:      http://localhost:${this.port}/api/price?symbol=agg:spot:ETHUSDT`);
    log.info(`  REST:      http://localhost:${this.port}/api/candles?symbol=ETHUSDT&timeframe=60&limit=100`);
    log.info(`  WebSocket: ws://localhost:${this.port}/ws (single connection, multiple subscriptions)`);
    log.info(`  Info:      http://localhost:${this.port}/info`);
  }
