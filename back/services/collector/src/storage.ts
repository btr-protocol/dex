...
import type { PairSymbol, OHLCCandle, SourceMap } from './types';
...
export class OHLCStorage {
  ...
  async initialize(): Promise<void> {
    ...
    log.info('Storage initialized (M1 only)');
  }
  ...
  async fillHistoricalData(pair: string, sources: SourceMap): Promise<void> {
    ...
    const gaps = this.findGaps(existing, targetStart, now);
      ...
      if (!gaps.length) {
        log.info(`${pair}: Complete (${existing.length} M1 candles)`);
        return;
      }

      const totalHours = gaps.reduce((s, g) => s + (g.end - g.start), 0) / 3_600_000;
      log.info(`${pair}: ${existing.length} candles, ${gaps.length} gap(s) (~${totalHours.toFixed(1)}h to fill)`);
      ...
      for (const gap of gaps) {
        ...
        log.info(`  ${pair}: Gap ${gapStart}-${gapEnd} (${gapMins}m)`);

        const candles = await this.fetchFromSources(pair, sources, gap.start, gap.end);
        if (candles.length) {
          this.saveCandles(pair, candles);
          log.info(`  ${pair}: +${candles.length} candles filled`);
      } else {
        log.warn(`  ${pair}: No candles fetched for gap!`);
      }
    }
  }
  ...
  private async fetchFromSources(pair: string, sources: SourceMap, start: number, end: number): Promise<OHLCCandle[]> {
    ...
    await Promise.all(Object.entries(sources).map(async ([key, { weight }]) => {
      ...
      log.info(`Rate limited ${key}: ${e.message}`);
      throw e;
    }
    }));
  ...
  static async fillAll(
    ...
    log.info(`Initializing ${exchangeIds.size} exchanges...`);
    for (const id of exchangeIds) {
      try { await storage.getExchange(id); log.info(`  ✓ ${id}`); }
      catch { log.info(`  ✗ ${id}`); }
    }
