...
async function testCollector() {
  console.log('=== Market Data Collector Test ===\n');

  const collector = new MarketCollector(config.tickers);

  try {
    // Start collector
    console.log('1. Starting collector...');
    await collector.start();

    // Wait for initial data
    console.log('2. Waiting 3 seconds for data collection...');
    await new Promise(resolve => setTimeout(resolve, 3000));

    // Test price retrieval
    console.log('3. Testing price retrieval:');
