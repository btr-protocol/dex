  ...
  async loadPoolAddresses(): Promise<void> {
    ...
    try {
      ...
      if (await file.exists()) {
        ...
        console.log('[pools] Loaded pool addresses from deployment.json');
        return;
      }
    } catch (e) {
      // Ignore
    ...
    if (poolZero && poolStable) {
      ...
      console.log('[pools] Loaded pool addresses from environment');
    } else {
      console.warn('[pools] No pool addresses configured - API will return empty data');
    }
  }
    ...
    const pools = await Promise.all(
      this.poolConfigs.map(async (config) => {
        try {
          ...
          console.error(`[pools] Failed to fetch ${config.name}:`, error);
          return null;
        }
      })
    );
