...
export class MarketCollector {
  ...
  private async startExchangeStream(
    ...
    if (!ctor) {
      error(`[${exchangeId}] Exchange not found`);
      return;
    }
    ...
    try {
      ...
      error(`[${exchangeId}] Failed to load markets: ${err.message}`);
      return;
    }
    ...
    while (!connection.stopSignal.stop && this.running) {
      try {
        ...
        if (!useIndividualWatch && connection.exchange.has?.watchTickers) {
          ...
          warn(`[${exchangeId}] No WebSocket support, using REST polling`);
          await this.pollExchangeREST(exchangeId, connection, subscriptions);
          await new Promise(r => setTimeout(r, 1000));
        }
      } catch (err: any) {
        error(`[${exchangeId}] WebSocket error: ${err.message}`);
  ...
  private async pollExchangeREST(
    ...
    try {
      ...
      error(`[${exchangeId}] REST poll error: ${err.message}`);
    }
  }
